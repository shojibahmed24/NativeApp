...
// Find the start of the first Animated.View entering={FadeInUp.delay(500)} style={styles.bottomControls}
// And find the matching end tag. Since regex can't easily parse nested tags, I will use string manipulation.

const startStr = `<Animated.View entering={FadeInUp.delay(500)} style={styles.bottomControls}>`;
const endStr = `</YStack>\n    </View>\n  );`;
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  const replacement = `<Animated.View entering={FadeInUp.delay(500)} style={styles.bottomControls}>
  ...
  console.log('Fixed syntax error in call screen');
} else {
