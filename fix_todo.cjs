...
// 1. Add To-Do option
const targetGallery = `<Text fontSize={12} color="#666">Gallery</Text>\n                </TouchableOpacity>`;
const todoOpt = `${targetGallery}
                
                <TouchableOpacity onPress={() => { setShowTaskModal(true); setShowScheduleOptions(false); }} style={{ alignItems: 'center', marginLeft: 20 }}>
...
if (!content.includes('To-Do</Text>')) {
    content = content.replace(targetGallery, todoOpt);
}
